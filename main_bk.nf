...
params.seqtk_path = '~/yan.a/software/seqtk/seqtk'
params.corset_path = '~/yan.a/software/corset-1.09-linux64/corset'
...
process LONG_READ_QUANT {
    ...
    # Minimap2 alignment
    minimap2 -ax map-ont -Y -p 1.0 -N 100 -I 1000G -t ${params.cpus} ${reformat_fasta} ${long_reads} | samtools view -@ ${params.cpus} -Sb > all_unsorted.bam
...
process CORSET {
    ...
    minimap2 -t ${params.cpus} -x asm5 -a -L -P --no-long-join -r2k -s40 -I 1000G ${reformat_fasta} ${reformat_fasta} | \\
...
/*
 * Process 9: Individual short read sample quantification
 */
process INDIVIDUAL_SHORT_QUANT {
    ...
    input:
    // This input will be set once and passed to every execution
    path reformat_fasta  // <-- CHANGE MADE HERE: Added 'set'
    // This input drives the execution (6 times)
...
/*
 * Process 10: Individual long read sample quantification
 */
process INDIVIDUAL_LONG_QUANT {
    ...
    # Minimap2 alignment
    minimap2 -ax map-ont -Y -p 1.0 -N 100 -I 1000G -t ${params.cpus} ${reformat_fasta} ${long_reads} | samtools view -@ ${params.cpus} -Sb > ${sample_name}_unsorted.bam
    ...
    rm ${sample_name}_sorted.bam ${sample_name}_sorted.bam.bai ${sample_name}_unsorted.bam ${sample_name}.primary.bam
    """
}
...
workflow {
    ...
    // Define the single-file channel outside the IF blocks
    def reformat_fasta_ch_single = REFORMAT_SEQUENCES.out.reformat_fasta
    ...
    if (params.short_list && (params.input_type == 'short' || params.input_type == 'hybrid')) {
        def short_list_lines = file(params.short_list).readLines().findAll { it.trim() }

        short_samples_ch = Channel.from(short_list_lines).map { line ->
            def read1_path = line.trim()
            def read1_file = file(read1_path)
            def sample_name = read1_file.name

            if (sample_name.endsWith(params.short_suffix1)) {
                sample_name = sample_name.replace(params.short_suffix1, "")
            } else {
                sample_name = read1_file.baseName
            }

            sample_name = "SR_" + sample_name
        ...
        // Now, you need to match the new 4-item tuple in the process definition
        INDIVIDUAL_SHORT_QUANT(reformat_fasta_ch_single, short_samples_ch)

    }

    if (params.long_list && (params.input_type == 'long' || params.input_type == 'hybrid')) {
        def long_list_lines = file(params.long_list).readLines().findAll { it.trim() }

        long_samples_ch = Channel.from(long_list_lines).map { line ->
            def long_read_path = line.trim()
            def long_read_file = file(long_read_path)
            def sample_name = long_read_file.name

            if (sample_name.endsWith(params.long_suffix)) {
                sample_name = sample_name.replace(params.long_suffix, "")
            } else {
                sample_name = long_read_file.baseName
            }

            sample_name = "LR_" + sample_name
            return tuple(sample_name, file(long_read_path))
        }

        // Now, you need to match the new 3-item tuple in the process definition
        INDIVIDUAL_LONG_QUANT(reformat_fasta_ch_single, long_samples_ch)
    }
}